...
Alternatively, install Qt Creator and open the `Droxne-qt.pro` file.

An executable named `Droxne-qt` will be built.
...
A warning for people using the *static binary* version of Droxne on a Linux/UNIX-ish system (tl;dr: **Berkely DB databases are not forward compatible**).

The static binary version of Droxne is linked against libdb5.3.
...
installed causes Droxne-qt to crash intermittently.  The issue has been reported as `launchpad bug 857790`_, but
isn't yet fixed.
