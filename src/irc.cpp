...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #DroxneTEST2\r");
            Send(hSocket, "WHO #DroxneTEST2\r");
        } else {
            // randomly join #Droxne00-#Droxne05
            // int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #Droxne%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #Droxne%02d\r", channel_number).c_str());
        }
